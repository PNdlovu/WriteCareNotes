...
const app = express();
const logger = new Logger('WriteCareNotes');
...
app.post('/wellness/track/:residentId', (req, res) => {
  ...
  logger.log(`Wellness tracking: ${emotion} for resident ${residentId}`);
...
app.post('/ai-documentation/generate', (req, res) => {
  ...
  logger.log(`Generating ${type} documentation for resident ${residentId}`);

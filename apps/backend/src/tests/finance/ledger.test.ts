 ...
 * @since 2025-01-01
 * 
 * @description Comprehensive test suite for double-entry ledger functionality
 * including unit tests, integration tests, and E2E tests.
 */

describe('Ledger Service', () => {
  letservice: LedgerService;
...
describe('Ledger Integration Tests', () => {
  letapp: any;
  letledgerService: LedgerService;
  ...
  describe('Ledger Workflow', () => {
    it('should complete full ledger workflow', async () => {
      ...
      const cashAccount = await ledgerService.createLedgerAccount(cashAccountData);
      const revenueAccount = await ledgerService.createLedgerAccount(revenueAccountData);
      ...
      const journalEntry = await ledgerService.createJournalEntry(journalEntryData);
      expect(journalEntry.status).toBe('draft');

      // 3. Post journal entry
      const postedJournalEntry = await ledgerService.postJournalEntry(
        journalEntry.id,
